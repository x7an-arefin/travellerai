...
import type { EntityIR, FieldIR, OperationIR } from '../lib/ir/types.js';

/**
 * @author arefin
 * @description Inspector utility to perform deep field-level diff between two specification files
 */
export async function diffSpecs(specPathA: string, specPathB: string): Promise<void> {
  ...
  if (errA.length > 0) throw new Error(`Spec A validation errors: ${errA.join(', ')}`);
  if (errB.length > 0) throw new Error(`Spec B validation errors: ${errB.join(', ')}`);
 ...
 */
function diffEntity(a: EntityIR, b: EntityIR): string[] {
 ...
 */
function diffField(a: FieldIR, b: FieldIR): string[] {
  const changes: string[] = [];
  if (a.type !== b.type) changes.push(chalk.yellow(`type: ${a.type} ➔ ${b.type}`));
  if (a.required !== b.required) changes.push(chalk.yellow(`required: ${a.required} ➔ ${b.required}`));
  if (a.unique !== b.unique) changes.push(chalk.yellow(`unique: ${a.unique} ➔ ${b.unique}`));
  if (String(a.default) !== String(b.default)) changes.push(chalk.yellow(`default: '${a.default}' ➔ '${b.default}'`));
  return changes;
}
