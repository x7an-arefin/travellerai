...
import { CustomerWalletController } from '@modules/customer-wallet/customer-wallet.controller.js';
import { CustomerWalletService } from '@modules/customer-wallet/customer-wallet.service.js';
import { CustomerWalletRepository } from '@modules/customer-wallet/customer-wallet.repository.js';
import { pre } from './update-customer-wallet.pre.js';
import { process } from './update-customer-wallet.process.js';
import { post } from './update-customer-wallet.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/customer-wallets'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('CustomerWallet — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a CustomerWallet in the database', async () => {
...
describe('CustomerWallet — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: CustomerWalletController,
   ...
   */
  it('PATCH /customer-wallets/:id responds with expected status', async () => {
    const res = await testApp.request('/customer-wallets/:id', {
      method: 'PATCH',
