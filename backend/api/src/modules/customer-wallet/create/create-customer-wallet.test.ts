...
import { pre } from './create-customer-wallet.pre.js';
import { process } from './create-customer-wallet.process.js';
import { post } from './create-customer-wallet.post.js';
...
describe('CustomerWallet — CREATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully create a CustomerWallet in the database', async () => {
...
describe('CustomerWallet — CREATE controller integration tests', () => {
   ...
   */
  it('POST /customer-wallets responds with expected status', async () => {
    const res = await testApp.request('/customer-wallets', {
      method: 'POST',
