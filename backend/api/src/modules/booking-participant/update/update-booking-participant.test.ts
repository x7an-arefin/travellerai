...
import { BookingParticipantController } from '@modules/booking-participant/booking-participant.controller.js';
import { BookingParticipantService } from '@modules/booking-participant/booking-participant.service.js';
import { BookingParticipantRepository } from '@modules/booking-participant/booking-participant.repository.js';
import { pre } from './update-booking-participant.pre.js';
import { process } from './update-booking-participant.process.js';
import { post } from './update-booking-participant.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/booking-participants'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('BookingParticipant — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a BookingParticipant in the database', async () => {
...
describe('BookingParticipant — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: BookingParticipantController,
   ...
   */
  it('PATCH /booking-participants/:id responds with expected status', async () => {
    const res = await testApp.request('/booking-participants/:id', {
      method: 'PATCH',
