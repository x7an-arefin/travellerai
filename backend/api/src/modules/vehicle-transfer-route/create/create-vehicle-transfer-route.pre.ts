...
/**
 * @author arefin
 * @description PRE lifecycle handler for CREATE VehicleTransferRoute — performs authentication, authorization, and pre-operation validation before the core logic runs
 */
export async function pre(ctx: LifecycleContext): Promise<LifecycleResult | void> {
  ...
  const hasPermission = await checkPermissions(session, ['vehicletransferroute:create']);
  if (!hasPermission) {
