...
import { pre } from './update-vehicle-transfer-route.pre.js';
import { process } from './update-vehicle-transfer-route.process.js';
import { post } from './update-vehicle-transfer-route.post.js';
...
describe('VehicleTransferRoute — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a VehicleTransferRoute in the database', async () => {
...
describe('VehicleTransferRoute — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /vehicle-transfer-routes/:id responds with expected status', async () => {
    const res = await testApp.request('/vehicle-transfer-routes/:id', {
      method: 'PATCH',
