...
import { pre } from './update-category.pre.js';
import { process } from './update-category.process.js';
import { post } from './update-category.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/categories'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('Category — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a Category in the database', async () => {
...
describe('Category — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /categories/:id responds with expected status', async () => {
    const res = await testApp.request('/categories/:id', {
      method: 'PATCH',
