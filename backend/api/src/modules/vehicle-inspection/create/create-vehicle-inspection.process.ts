...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for CREATE VehicleInspection — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new VehicleInspectionRepository();

  const vehicleInspection = await repo.create(ctx.input as any);
