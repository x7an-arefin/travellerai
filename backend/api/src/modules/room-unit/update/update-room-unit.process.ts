...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for UPDATE RoomUnit — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  ...
  if (!updated) {
    throw new AppError('UPDATE_FAILED', 'Failed to update RoomUnit', 500);
  }
