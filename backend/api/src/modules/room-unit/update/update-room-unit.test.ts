...
import { RoomUnitController } from '@modules/room-unit/room-unit.controller.js';
import { RoomUnitService } from '@modules/room-unit/room-unit.service.js';
import { RoomUnitRepository } from '@modules/room-unit/room-unit.repository.js';
import { pre } from './update-room-unit.pre.js';
import { process } from './update-room-unit.process.js';
import { post } from './update-room-unit.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/room-units'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('RoomUnit — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a RoomUnit in the database', async () => {
...
describe('RoomUnit — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /room-units/:id responds with expected status', async () => {
    const res = await testApp.request('/room-units/:id', {
      method: 'PATCH',
