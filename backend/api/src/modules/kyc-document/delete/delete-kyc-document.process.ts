...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for DELETE KycDocument — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  ...
  if (!deleted) {
    throw new AppError('DELETE_FAILED', 'Failed to delete KycDocument', 500);
  }
