...
/**
 * @author arefin
 * @description PRE lifecycle handler for LIST KycDocument — performs authentication, authorization, and pre-operation validation before the core logic runs
 */
export async function pre(ctx: LifecycleContext): Promise<LifecycleResult | void> {
  ...
  const hasPermission = await checkPermissions(session, ['kyc_document:list']);
  if (!hasPermission) {
