...
import { ProviderStaffController } from '@modules/provider-staff/provider-staff.controller.js';
import { ProviderStaffService } from '@modules/provider-staff/provider-staff.service.js';
import { ProviderStaffRepository } from '@modules/provider-staff/provider-staff.repository.js';
import { pre } from './update-provider-staff.pre.js';
import { process } from './update-provider-staff.process.js';
import { post } from './update-provider-staff.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/provider-staffs'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('ProviderStaff — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a ProviderStaff in the database', async () => {
...
describe('ProviderStaff — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: ProviderStaffController,
   ...
   */
  it('PATCH /provider-staffs/:id responds with expected status', async () => {
    const res = await testApp.request('/provider-staffs/:id', {
      method: 'PATCH',
