...
import { DestinationController } from '@modules/destination/destination.controller.js';
import { DestinationService } from '@modules/destination/destination.service.js';
import { DestinationRepository } from '@modules/destination/destination.repository.js';
import { pre } from './update-destination.pre.js';
import { process } from './update-destination.process.js';
import { post } from './update-destination.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/destinations'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('Destination — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a Destination in the database', async () => {
...
describe('Destination — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /destinations/:id responds with expected status', async () => {
    const res = await testApp.request('/destinations/:id', {
      method: 'PATCH',
