...
import { LoyaltyAccountController } from '@modules/loyalty-account/loyalty-account.controller.js';
import { LoyaltyAccountService } from '@modules/loyalty-account/loyalty-account.service.js';
import { LoyaltyAccountRepository } from '@modules/loyalty-account/loyalty-account.repository.js';
import { pre } from './update-loyalty-account.pre.js';
import { process } from './update-loyalty-account.process.js';
import { post } from './update-loyalty-account.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/loyalty-accounts'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('LoyaltyAccount — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a LoyaltyAccount in the database', async () => {
...
describe('LoyaltyAccount — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: LoyaltyAccountController,
   ...
   */
  it('PATCH /loyalty-accounts/:id responds with expected status', async () => {
    const res = await testApp.request('/loyalty-accounts/:id', {
      method: 'PATCH',
