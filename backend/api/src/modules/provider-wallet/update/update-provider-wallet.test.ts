...
import { ProviderWalletController } from '@modules/provider-wallet/provider-wallet.controller.js';
import { ProviderWalletService } from '@modules/provider-wallet/provider-wallet.service.js';
import { ProviderWalletRepository } from '@modules/provider-wallet/provider-wallet.repository.js';
import { pre } from './update-provider-wallet.pre.js';
import { process } from './update-provider-wallet.process.js';
import { post } from './update-provider-wallet.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/provider-wallets'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('ProviderWallet — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a ProviderWallet in the database', async () => {
...
describe('ProviderWallet — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /provider-wallets/:id responds with expected status', async () => {
    const res = await testApp.request('/provider-wallets/:id', {
      method: 'PATCH',
