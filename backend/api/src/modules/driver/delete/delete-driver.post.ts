...
/**
 * @author arefin
 * @description POST lifecycle handler for DELETE Driver — publishes domain events and performs post-operation side effects after the core logic succeeds
 */
export async function post(ctx: LifecycleContext): Promise<void> {
  ...
  await publishEvent(ctx.env.DOMAIN_EVENTS, {
    eventName: DRIVER_EVENTS.DELETED,
