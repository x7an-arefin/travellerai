...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for GET ExchangeRate — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new ExchangeRateRepository();

  const exchangeRate = await repo.findById(ctx.input.id as string);
  if (!exchangeRate) {
