...
import { VehiclePricingPlanController } from '@modules/vehicle-pricing-plan/vehicle-pricing-plan.controller.js';
import { VehiclePricingPlanService } from '@modules/vehicle-pricing-plan/vehicle-pricing-plan.service.js';
import { VehiclePricingPlanRepository } from '@modules/vehicle-pricing-plan/vehicle-pricing-plan.repository.js';
import { pre } from './update-vehicle-pricing-plan.pre.js';
import { process } from './update-vehicle-pricing-plan.process.js';
import { post } from './update-vehicle-pricing-plan.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/vehicle-pricing-plans'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('VehiclePricingPlan — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a VehiclePricingPlan in the database', async () => {
...
describe('VehiclePricingPlan — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: VehiclePricingPlanController,
   ...
   */
  it('PATCH /vehicle-pricing-plans/:id responds with expected status', async () => {
    const res = await testApp.request('/vehicle-pricing-plans/:id', {
      method: 'PATCH',
