...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for DELETE VehiclePricingPlan — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  ...
  if (!deleted) {
    throw new AppError('DELETE_FAILED', 'Failed to delete VehiclePricingPlan', 500);
  }
