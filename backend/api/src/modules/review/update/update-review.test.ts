...
import { pre } from './update-review.pre.js';
import { process } from './update-review.process.js';
import { post } from './update-review.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/reviews'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('Review — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a Review in the database', async () => {
...
describe('Review — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /reviews/:id responds with expected status', async () => {
    const res = await testApp.request('/reviews/:id', {
      method: 'PATCH',
