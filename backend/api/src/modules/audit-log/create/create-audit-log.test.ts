...
import { pre } from './create-audit-log.pre.js';
import { process } from './create-audit-log.process.js';
import { post } from './create-audit-log.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/audit-logs'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('AuditLog — CREATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully create a AuditLog in the database', async () => {
...
describe('AuditLog — CREATE controller integration tests', () => {
   ...
   */
  it('POST /audit-logs responds with expected status', async () => {
    const res = await testApp.request('/audit-logs', {
      method: 'POST',
