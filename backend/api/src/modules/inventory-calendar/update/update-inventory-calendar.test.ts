...
import { InventoryCalendarController } from '@modules/inventory-calendar/inventory-calendar.controller.js';
import { InventoryCalendarService } from '@modules/inventory-calendar/inventory-calendar.service.js';
import { InventoryCalendarRepository } from '@modules/inventory-calendar/inventory-calendar.repository.js';
import { pre } from './update-inventory-calendar.pre.js';
import { process } from './update-inventory-calendar.process.js';
import { post } from './update-inventory-calendar.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/inventory-calendars'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('InventoryCalendar — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a InventoryCalendar in the database', async () => {
...
describe('InventoryCalendar — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: InventoryCalendarController,
   ...
   */
  it('PATCH /inventory-calendars/:id responds with expected status', async () => {
    const res = await testApp.request('/inventory-calendars/:id', {
      method: 'PATCH',
