...
import { BookingAddonItemController } from '@modules/booking-addon-item/booking-addon-item.controller.js';
import { BookingAddonItemService } from '@modules/booking-addon-item/booking-addon-item.service.js';
import { BookingAddonItemRepository } from '@modules/booking-addon-item/booking-addon-item.repository.js';
import { pre } from './update-booking-addon-item.pre.js';
import { process } from './update-booking-addon-item.process.js';
import { post } from './update-booking-addon-item.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/booking-addon-items'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('BookingAddonItem — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a BookingAddonItem in the database', async () => {
...
describe('BookingAddonItem — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: BookingAddonItemController,
   ...
   */
  it('PATCH /booking-addon-items/:id responds with expected status', async () => {
    const res = await testApp.request('/booking-addon-items/:id', {
      method: 'PATCH',
