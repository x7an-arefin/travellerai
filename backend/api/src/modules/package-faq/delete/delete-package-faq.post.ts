...
/**
 * @author arefin
 * @description POST lifecycle handler for DELETE PackageFaq — publishes domain events and performs post-operation side effects after the core logic succeeds
 */
export async function post(ctx: LifecycleContext): Promise<void> {
  ...
  await publishEvent(ctx.env.DOMAIN_EVENTS, {
    eventName: PACKAGE_FAQ_EVENTS.DELETED,
