...
import { GuideProfileController } from '@modules/guide-profile/guide-profile.controller.js';
import { GuideProfileService } from '@modules/guide-profile/guide-profile.service.js';
import { GuideProfileRepository } from '@modules/guide-profile/guide-profile.repository.js';
import { pre } from './update-guide-profile.pre.js';
import { process } from './update-guide-profile.process.js';
import { post } from './update-guide-profile.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/guide-profiles'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('GuideProfile — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a GuideProfile in the database', async () => {
...
describe('GuideProfile — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: GuideProfileController,
   ...
   */
  it('PATCH /guide-profiles/:id responds with expected status', async () => {
    const res = await testApp.request('/guide-profiles/:id', {
      method: 'PATCH',
