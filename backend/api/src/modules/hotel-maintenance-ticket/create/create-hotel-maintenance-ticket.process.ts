...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for CREATE HotelMaintenanceTicket — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new HotelMaintenanceTicketRepository();

  const hotelMaintenanceTicket = await repo.create(ctx.input as any);
