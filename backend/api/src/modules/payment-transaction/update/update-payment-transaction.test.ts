...
import { PaymentTransactionController } from '@modules/payment-transaction/payment-transaction.controller.js';
import { PaymentTransactionService } from '@modules/payment-transaction/payment-transaction.service.js';
import { PaymentTransactionRepository } from '@modules/payment-transaction/payment-transaction.repository.js';
import { pre } from './update-payment-transaction.pre.js';
import { process } from './update-payment-transaction.process.js';
import { post } from './update-payment-transaction.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/payment-transactions'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('PaymentTransaction — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a PaymentTransaction in the database', async () => {
...
describe('PaymentTransaction — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /payment-transactions/:id responds with expected status', async () => {
    const res = await testApp.request('/payment-transactions/:id', {
      method: 'PATCH',
