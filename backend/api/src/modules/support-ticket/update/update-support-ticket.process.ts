...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for UPDATE SupportTicket — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  ...
  if (!updated) {
    throw new AppError('UPDATE_FAILED', 'Failed to update SupportTicket', 500);
  }
