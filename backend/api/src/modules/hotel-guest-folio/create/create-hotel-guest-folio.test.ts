...
import { pre } from './create-hotel-guest-folio.pre.js';
import { process } from './create-hotel-guest-folio.process.js';
import { post } from './create-hotel-guest-folio.post.js';
...
describe('HotelGuestFolio — CREATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully create a HotelGuestFolio in the database', async () => {
...
describe('HotelGuestFolio — CREATE controller integration tests', () => {
   ...
   */
  it('POST /hotel-guest-folios responds with expected status', async () => {
    const res = await testApp.request('/hotel-guest-folios', {
      method: 'POST',
