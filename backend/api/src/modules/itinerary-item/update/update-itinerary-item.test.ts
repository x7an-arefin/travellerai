...
import { ItineraryItemController } from '@modules/itinerary-item/itinerary-item.controller.js';
import { ItineraryItemService } from '@modules/itinerary-item/itinerary-item.service.js';
import { ItineraryItemRepository } from '@modules/itinerary-item/itinerary-item.repository.js';
import { pre } from './update-itinerary-item.pre.js';
import { process } from './update-itinerary-item.process.js';
import { post } from './update-itinerary-item.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/itinerary-items'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('ItineraryItem — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a ItineraryItem in the database', async () => {
...
describe('ItineraryItem — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: ItineraryItemController,
   ...
   */
  it('PATCH /itinerary-items/:id responds with expected status', async () => {
    const res = await testApp.request('/itinerary-items/:id', {
      method: 'PATCH',
