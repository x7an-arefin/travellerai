...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for GET ItineraryItem — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new ItineraryItemRepository();

  const itineraryItem = await repo.findById(ctx.input.id as string);
  if (!itineraryItem) {
