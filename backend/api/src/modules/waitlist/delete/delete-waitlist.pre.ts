...
/**
 * @author arefin
 * @description PRE lifecycle handler for DELETE Waitlist — performs authentication, authorization, and pre-operation validation before the core logic runs
 */
export async function pre(ctx: LifecycleContext): Promise<LifecycleResult | void> {
  ...
  const hasPermission = await checkPermissions(session, ['waitlist:delete']);
  if (!hasPermission) {
