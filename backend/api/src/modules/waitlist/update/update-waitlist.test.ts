...
import { pre } from './update-waitlist.pre.js';
import { process } from './update-waitlist.process.js';
import { post } from './update-waitlist.post.js';
...
describe('Waitlist — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a Waitlist in the database', async () => {
...
describe('Waitlist — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /waitlists/:id responds with expected status', async () => {
    const res = await testApp.request('/waitlists/:id', {
      method: 'PATCH',
