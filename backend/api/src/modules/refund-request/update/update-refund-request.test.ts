...
import { RefundRequestController } from '@modules/refund-request/refund-request.controller.js';
import { RefundRequestService } from '@modules/refund-request/refund-request.service.js';
import { RefundRequestRepository } from '@modules/refund-request/refund-request.repository.js';
import { pre } from './update-refund-request.pre.js';
import { process } from './update-refund-request.process.js';
import { post } from './update-refund-request.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/refund-requests'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('RefundRequest — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a RefundRequest in the database', async () => {
...
describe('RefundRequest — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: RefundRequestController,
   ...
   */
  it('PATCH /refund-requests/:id responds with expected status', async () => {
    const res = await testApp.request('/refund-requests/:id', {
      method: 'PATCH',
