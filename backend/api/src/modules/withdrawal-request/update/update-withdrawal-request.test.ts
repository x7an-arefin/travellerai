...
import { WithdrawalRequestController } from '@modules/withdrawal-request/withdrawal-request.controller.js';
import { WithdrawalRequestService } from '@modules/withdrawal-request/withdrawal-request.service.js';
import { WithdrawalRequestRepository } from '@modules/withdrawal-request/withdrawal-request.repository.js';
import { pre } from './update-withdrawal-request.pre.js';
import { process } from './update-withdrawal-request.process.js';
import { post } from './update-withdrawal-request.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/withdrawal-requests'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('WithdrawalRequest — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a WithdrawalRequest in the database', async () => {
...
describe('WithdrawalRequest — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: WithdrawalRequestController,
   ...
   */
  it('PATCH /withdrawal-requests/:id responds with expected status', async () => {
    const res = await testApp.request('/withdrawal-requests/:id', {
      method: 'PATCH',
