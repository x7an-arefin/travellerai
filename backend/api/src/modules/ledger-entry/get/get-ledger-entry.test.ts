...
import { pre } from './get-ledger-entry.pre.js';
import { process } from './get-ledger-entry.process.js';
import { post } from './get-ledger-entry.post.js';
...
describe('LedgerEntry — GET lifecycle unit tests', () => {
...
describe('LedgerEntry — GET controller integration tests', () => {
   ...
   */
  it('GET /ledger-entries/:id responds with expected status', async () => {
    const res = await testApp.request('/ledger-entries/:id', {
      method: 'GET',
