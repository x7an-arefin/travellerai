...
/**
 * @author arefin
 * @description PRE lifecycle handler for CREATE LedgerEntry — performs authentication, authorization, and pre-operation validation before the core logic runs
 */
export async function pre(ctx: LifecycleContext): Promise<LifecycleResult | void> {
  ...
  const hasPermission = await checkPermissions(session, ['ledger_entry:create']);
  if (!hasPermission) {
