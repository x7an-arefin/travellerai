...
/**
 * @author arefin
 * @description PRE lifecycle handler for UPDATE GiftCard — performs authentication, authorization, and pre-operation validation before the core logic runs
 */
export async function pre(ctx: LifecycleContext): Promise<LifecycleResult | void> {
  ...
  const hasPermission = await checkPermissions(session, ['gift_card:update']);
  if (!hasPermission) {
