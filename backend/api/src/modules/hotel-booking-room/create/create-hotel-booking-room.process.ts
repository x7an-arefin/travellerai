...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for CREATE HotelBookingRoom — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new HotelBookingRoomRepository();

  const hotelBookingRoom = await repo.create(ctx.input as any);
