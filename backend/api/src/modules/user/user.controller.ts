...
import { UpdateUserInputSchema } from './update/update-user.input.js';

import { DeleteUserInputSchema } from './delete/delete-user.input.js';
...
export class UserController {
  ...
  /**
   * @author arefin
   * @description Handle PATCH /api/v1/users/:id — validate input and delegate to update lifecycle
   */
  @UseGuards(AuthGuard)
  @Patch('/:id')
  async update(
    ...
    const inputResult = UpdateUserInputSchema.safeParse(body);
    if (!inputResult.success) {
      return c.json({ error: 'VALIDATION_ERROR', details: inputResult.error.flatten().fieldErrors }, 422);
    }
  ...
  /**
   * @author arefin
   * @description Handle DELETE /api/v1/users/:id — validate input and delegate to delete lifecycle
   */
  @UseGuards(AuthGuard)
  @Delete('/:id')
  async delete(
