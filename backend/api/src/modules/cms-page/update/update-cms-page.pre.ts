...
/**
 * @author arefin
 * @description PRE lifecycle handler for UPDATE CmsPage — performs authentication, authorization, and pre-operation validation before the core logic runs
 */
export async function pre(ctx: LifecycleContext): Promise<LifecycleResult | void> {
  ...
  const hasPermission = await checkPermissions(session, ['cms_page:update']);
  if (!hasPermission) {
