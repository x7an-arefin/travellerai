...
import { pre } from './update-booking.pre.js';
import { process } from './update-booking.process.js';
import { post } from './update-booking.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/bookings'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('Booking — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a Booking in the database', async () => {
...
describe('Booking — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /bookings/:id responds with expected status', async () => {
    const res = await testApp.request('/bookings/:id', {
      method: 'PATCH',
