...
/**
 * @author arefin
 * @description POST lifecycle handler for CREATE HotelReview — publishes domain events and performs post-operation side effects after the core logic succeeds
 */
export async function post(ctx: LifecycleContext): Promise<void> {
  ...
  await publishEvent(ctx.env.DOMAIN_EVENTS, {
    eventName: HOTEL_REVIEW_EVENTS.CREATED,
