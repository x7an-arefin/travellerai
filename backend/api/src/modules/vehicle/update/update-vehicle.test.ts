...
import { pre } from './update-vehicle.pre.js';
import { process } from './update-vehicle.process.js';
import { post } from './update-vehicle.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/vehicles'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('Vehicle — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a Vehicle in the database', async () => {
...
describe('Vehicle — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /vehicles/:id responds with expected status', async () => {
    const res = await testApp.request('/vehicles/:id', {
      method: 'PATCH',
