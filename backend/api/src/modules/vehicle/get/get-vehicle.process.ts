import type { LifecycleContext } from '@core/lifecycle/lifecycle-context.js';
import type { LifecycleResult } from '@core/lifecycle/lifecycle-result.js';
import { VehicleRepository } from '@modules/vehicle/vehicle.repository.js';
import { AppError } from '@core/errors/application-error.js';


/**
 * @author arefin
 * @description PROCESS lifecycle handler for GET Vehicle — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new VehicleRepository();

  const vehicle = await repo.findById(ctx.input.id as string);
  if (!vehicle) {
    throw new AppError('NOT_FOUND', 'Vehicle not found', 404);
  }

  return { output: vehicle, entityId: vehicle.id };

}
