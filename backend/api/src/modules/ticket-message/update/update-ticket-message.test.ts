...
import { TicketMessageController } from '@modules/ticket-message/ticket-message.controller.js';
import { TicketMessageService } from '@modules/ticket-message/ticket-message.service.js';
import { TicketMessageRepository } from '@modules/ticket-message/ticket-message.repository.js';
import { pre } from './update-ticket-message.pre.js';
import { process } from './update-ticket-message.process.js';
import { post } from './update-ticket-message.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/ticket-messages'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('TicketMessage — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a TicketMessage in the database', async () => {
...
describe('TicketMessage — UPDATE controller integration tests', () => {
   ...
   */
  it('PATCH /ticket-messages/:id responds with expected status', async () => {
    const res = await testApp.request('/ticket-messages/:id', {
      method: 'PATCH',
