...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for GET TicketMessage — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new TicketMessageRepository();

  const ticketMessage = await repo.findById(ctx.input.id as string);
  if (!ticketMessage) {
