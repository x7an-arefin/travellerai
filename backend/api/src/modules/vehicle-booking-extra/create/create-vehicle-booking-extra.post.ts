...
/**
 * @author arefin
 * @description POST lifecycle handler for CREATE VehicleBookingExtra — publishes domain events and performs post-operation side effects after the core logic succeeds
 */
export async function post(ctx: LifecycleContext): Promise<void> {
  ...
  await publishEvent(ctx.env.DOMAIN_EVENTS, {
    eventName: VEHICLE_BOOKING_EXTRA_EVENTS.CREATED,
