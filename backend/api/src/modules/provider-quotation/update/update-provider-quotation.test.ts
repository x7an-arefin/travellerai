...
import { ProviderQuotationController } from '@modules/provider-quotation/provider-quotation.controller.js';
import { ProviderQuotationService } from '@modules/provider-quotation/provider-quotation.service.js';
import { ProviderQuotationRepository } from '@modules/provider-quotation/provider-quotation.repository.js';
import { pre } from './update-provider-quotation.pre.js';
import { process } from './update-provider-quotation.process.js';
import { post } from './update-provider-quotation.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/provider-quotations'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('ProviderQuotation — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a ProviderQuotation in the database', async () => {
...
describe('ProviderQuotation — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: ProviderQuotationController,
   ...
   */
  it('PATCH /provider-quotations/:id responds with expected status', async () => {
    const res = await testApp.request('/provider-quotations/:id', {
      method: 'PATCH',
