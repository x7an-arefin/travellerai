...
import { ProviderPayoutAccountController } from '@modules/provider-payout-account/provider-payout-account.controller.js';
import { ProviderPayoutAccountService } from '@modules/provider-payout-account/provider-payout-account.service.js';
import { ProviderPayoutAccountRepository } from '@modules/provider-payout-account/provider-payout-account.repository.js';
import { pre } from './update-provider-payout-account.pre.js';
import { process } from './update-provider-payout-account.process.js';
import { post } from './update-provider-payout-account.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/provider-payout-accounts'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('ProviderPayoutAccount — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a ProviderPayoutAccount in the database', async () => {
...
describe('ProviderPayoutAccount — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: ProviderPayoutAccountController,
   ...
   */
  it('PATCH /provider-payout-accounts/:id responds with expected status', async () => {
    const res = await testApp.request('/provider-payout-accounts/:id', {
      method: 'PATCH',
