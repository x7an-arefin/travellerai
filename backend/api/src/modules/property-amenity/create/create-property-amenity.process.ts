...
/**
 * @author arefin
 * @description PROCESS lifecycle handler for CREATE PropertyAmenity — executes the core business operation via the repository
 */
export async function process(ctx: LifecycleContext): Promise<LifecycleResult> {
  const repo = new PropertyAmenityRepository();

  const propertyAmenity = await repo.create(ctx.input as any);
