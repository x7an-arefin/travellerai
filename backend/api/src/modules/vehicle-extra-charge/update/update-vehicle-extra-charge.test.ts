...
import { VehicleExtraChargeController } from '@modules/vehicle-extra-charge/vehicle-extra-charge.controller.js';
import { VehicleExtraChargeService } from '@modules/vehicle-extra-charge/vehicle-extra-charge.service.js';
import { VehicleExtraChargeRepository } from '@modules/vehicle-extra-charge/vehicle-extra-charge.repository.js';
import { pre } from './update-vehicle-extra-charge.pre.js';
import { process } from './update-vehicle-extra-charge.process.js';
import { post } from './update-vehicle-extra-charge.post.js';
...
function makeCtx(overrides: Partial<LifecycleContext> = {}): LifecycleContext {
  return {
    ...
    request: new Request('https://example.com/vehicle-extra-charges'),
    input: {},
    result: null,
    meta: {},
    ...overrides,
...
describe('VehicleExtraCharge — UPDATE lifecycle unit tests', () => {
  ...
  describe('process()', () => {
     ...
     */
    it('should successfully update a VehicleExtraCharge in the database', async () => {
...
describe('VehicleExtraCharge — UPDATE controller integration tests', () => {
  ...
  beforeEach(async () => {
    testApp = await createControllerTestApplication({
      controller: VehicleExtraChargeController,
   ...
   */
  it('PATCH /vehicle-extra-charges/:id responds with expected status', async () => {
    const res = await testApp.request('/vehicle-extra-charges/:id', {
      method: 'PATCH',
