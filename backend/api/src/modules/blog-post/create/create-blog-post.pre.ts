...
/**
 * @author arefin
 * @description PRE lifecycle handler for CREATE BlogPost — performs authentication, authorization, and pre-operation validation before the core logic runs
 */
export async function pre(ctx: LifecycleContext): Promise<LifecycleResult | void> {
  ...
  const hasPermission = await checkPermissions(session, ['blog_post:create']);
  if (!hasPermission) {
